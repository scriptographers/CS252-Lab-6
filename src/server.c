...
struct timeval diffTimeval(struct timeval t2, struct timeval t1) {
  ...
  // https://stackoverflow.com/questions/32534392/timing-response-time-from-server
  ...
  // microsecond result could be negative, ex. 2.1 - 1.9 = 1 sec - 800000
  // microseconds if so, subtract one second and add 1000000 microseconds
...
int main() {
  ...
  int new_sockfd = accept(sockfd, (struct sockaddr *)&client_addr, &len_caddr);
  if (new_sockfd < 0) {
